...
abstract contract BlindBackrunLogic is Ownable {
    ...
    function getAmountIn(
        ...
        uint256 amountIn = (numerator * 1000) / denominator;
    ...
    function getNumerator(
        ...
        if (firstPairData.isWETHZero == true) {
            ...
            uint256 numerator = (sqrt(presqrt) - 1e3) *
                secondPairData.reserve1 *
                firstPairData.reserve0;
    ...
    function getDenominator(
        ...
        if (firstPairData.isWETHZero == true) {
            uint256 denominator = (uniswappyFee *
                secondPairData.reserve1 *
                1000) + (uniswappyFee * uniswappyFee * firstPairData.reserve1);
            return denominator;
        } else {
            uint256 denominator = (uniswappyFee *
                secondPairData.reserve0 *
                1000) + (uniswappyFee * uniswappyFee * firstPairData.reserve0);
            return denominator;
        }
    }
    ...
    function getPairData(
        IUniswapV2Pair pair
    ) private view returns (IPairReserves.PairReserves memory) {
